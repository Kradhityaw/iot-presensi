...
def sync_rfid_cache():
    """Mengunduh UID kartu, NAMA, dan DEPARTEMEN pengguna lalu menyimpannya di cache."""
    global rfid_cache, organization_name
    print("Mulai sinkronisasi cache RFID, Nama, & Dept...")
    # Tampilkan pesan di OLED (gunakan fungsi yang sudah ada)
    oled_center_multiline_vertical_stretch(["Sinkronisasi", "Ke Server..."])
    
    # Ambil nama organisasi (logika ini sudah benar)
    try:
        org_url = f"{SUPABASE_URL}/rest/v1/organizations?id=eq.{organization_id}&select=name"
        headers = {'apikey': SUPABASE_KEY, 'Authorization': f'Bearer {SUPABASE_KEY}'}
        org_response = urequests.get(org_url, headers=headers)
        ...
        print(f"Error mengambil data organisasi: {e}")
        organization_name = "Unknown Org"
    
    # --- PERBAIKAN UTAMA DIMULAI DI SINI ---

    # Query yang diperbarui dengan HINT untuk memberitahu relasi mana yang harus digunakan
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            rfid_cache = {}
            for item in data:
                # Ambil nama (gunakan .get() untuk keamanan)
                try:
            ...
            print(f"Gagal sinkronisasi. Status: {response.status_code}, Pesan: {response.text}")
            oled_center_multiline_vertical_stretch(["Sinkronisasi", "Gagal!"])
            danger()
        response.close()
    except Exception as e:
        print(f"Error saat sinkronisasi cache: {e}")
        danger()
